...
Build a backend-driven UI Composition Platform where the backend owns
composition, routing, authorization, data orchestration, configuration,
localization and feature flags. The frontend is a render-only engine.
...
HTTP Request → Authentication → Build immutable RenderContext → Resolve
Application → Resolve Route → Resolve Component Tree → Evaluate Policies
→ Resolve Configuration → Resolve Data → Apply Plugins → Serialize UI
JSON → Frontend Renderer
...
Before implementing any feature: - Is it metadata? - Is it immutable? -
Can it be a plugin? - Can it be a strategy? - Can it be an event? - Can
it be a pipeline stage? - Is it versionable? - Is it tenant-aware? - Is
it observable? - Does it keep the UI dumb?
