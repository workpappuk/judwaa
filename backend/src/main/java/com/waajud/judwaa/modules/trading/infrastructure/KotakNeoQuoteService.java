...
public class KotakNeoQuoteService {
	...
	public KotakNeoQuoteService(RestTemplate restTemplate, KotakProperties props, ObjectMapper objectMapper) {
	...
	public List<QuoteResponse> fetchQuotes(List<String> neoSymbols) {
		...
		// nse_cm%7CNifty%2050
		String encodedSymbols = neoSymbols.stream().map(this::encodePathPart).collect(Collectors.joining(","));
		...
		ResponseEntity<QuoteResponse[]> response = restTemplate.exchange(url, HttpMethod.GET, request,
				QuoteResponse[].class);
