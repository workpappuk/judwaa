...
test('lms hub navigates to each management page', async ({ page }) => {
  ...
  await page.getByRole('link', { name: /organization management/i }).first().click();
  ...
  await page.goto(`${APP_BASE_URL}/lms`);
  await page.getByRole('link', { name: /school routes/i }).first().click();
  await expect(page).toHaveURL(/\/lms\/school-routes$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.getByRole('link', { name: /student management/i }).first().click();
  await expect(page).toHaveURL(/\/lms\/student-management$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.getByRole('link', { name: /exam management/i }).first().click();
