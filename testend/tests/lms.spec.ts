...
test('lms hub navigates to each management page', async ({ page }) => {
  ...
  await page.locator('main a[href="/lms/organization-management"]').first().click();
  await expect(page).toHaveURL(/\/lms\/organization-management$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.locator('main a[href="/lms/school-management"]').first().click();
  await expect(page).toHaveURL(/\/lms\/school-management$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.locator('main a[href="/lms/school-routes"]').first().click();
  await expect(page).toHaveURL(/\/lms\/school-routes$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.locator('main a[href="/lms/student-management"]').first().click();
  await expect(page).toHaveURL(/\/lms\/student-management$/);

  await page.goto(`${APP_BASE_URL}/lms`);
  await page.locator('main a[href="/lms/exam-management"]').first().click();
